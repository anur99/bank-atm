...
public class ChangeNameGUI {
	...
	public ChangeNameGUI() {
		...
		oldLabel = new JLabel("Enter Current Name");
		...
		newLabel = new JLabel("Enter New Name");
	...
	private void submitBtnActionPerformed(ActionEvent e) {
		String newName = newText.getText().toString();
		String oldName = oldText.getText().toString();
		String checkOldName="";
		try {
			...
			while(rs.next()) {
				checkOldName = rs.getString(1);
				break;
			}
			if(checkOldName.equals(oldName)) {
				PreparedStatement psm2 = con.prepareStatement("update users_account set name=? where accountNo="+InfoGUI.confirm);
				psm2.setString(1, newName);
				psm2.executeUpdate();
				...
				JLabel emailLabel = new JLabel("Name Successfully Change.....");
				...
				JOptionPane.showMessageDialog(null, "Name doesn't match\nplease type again");
				oldText.setText("");
